#include "bytemacro.h"
unsigned int __cdecl byteCount(unsigned char* hay, unsigned char* ned, unsigned int haylen, unsigned int nedlen, unsigned int overlap){	
	// specify overlap = 1 to count overlapping occurances, eg in hay='aaa', the needle 'aa' is found twice: [aa]a and a[aa].
	// for byteCount(), compiled with gcc -Ofast
	unsigned int i=0;
	unsigned int n=0;
	unsigned int j;
	unsigned int rem;
